    ...
    <table class="table table-bordered">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $user->email }}</td> <!-- Fixed: Removed $ symbol -->
          <td>
          ...
          <td>
            @if(Auth::check())
            <form action="{{ route('adminusers.remove', $user->id) }}" method="POST" class="d-inline-block" onsubmit="return confirmDelete('{{ $user->name ?? 'this user' }}')">
              @csrf
              @method('DELETE')
              <button type='submit' class="remove-link text-danger">Remove user</button>
            </form>
            @else
