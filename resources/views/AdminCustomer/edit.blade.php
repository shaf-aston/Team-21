    ...
    <form method="POST" action="{{ route('adminuser.update', $user->id)}}">
        ...
        <div class="form-group">
            ...
            <div class="name-group">
                <input type="text" id="name" name="name" placeholder="First Name" value="{{ old('name', $user->name) }}" required>
                @error('name')
        ...
        <div class="form-group">
            ...
            <input type="email" id="email" name="email" placeholder="e.g. hello@gmail.com" value="{{ old('email', $user->email) }}" required>
            @error('email')
                <p class="error-message" style="color: red;">{{ $message }}</p>
            @enderror
        </div>
